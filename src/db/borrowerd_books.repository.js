...
export const borrowedBooksRepository = {
  ...
  findAll: async ({
    ...
    if (bookId) {
      if (queryStr.includes('WHERE')) queryStr += ` AND bb.book = $${params.length + 1}`
    ...
    if (lastMonth) {
      if (queryStr.includes('WHERE')) queryStr += ` AND bb.checked_out_date > NOW() - INTERVAL '1 month'`
      else queryStr += ` WHERE bb.checked_out_date > NOW() - INTERVAL '1 month'`
    }

    const totalCount = await query(queryStr.replace('bb.*, b.title, b.author, br.name, br.email', 'COUNT(*)'), params)
    ...
    if (queryStr.includes('WHERE')) queryStr = queryStr.replace('WHERE', joinStatement + ' WHERE')
    else queryStr += joinStatement


    queryStr += ` ORDER BY bb.${sort} ${sortDirection === "1" ? "ASC" : "DESC"} LIMIT $${params.length + 1} OFFSET $${params.length + 2}`
